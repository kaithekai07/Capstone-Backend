...
def append_to_gcs_master(df_new, section_name):
    ...
    master_blob = bucket.blob(f"{section_name}/master.csv")

    try:
        existing_data = master_blob.download_as_text()
        df_existing = pd.read_csv(StringIO(existing_data), header=None)
        df_combined = pd.concat([df_existing, df_new], ignore_index=True)
    except Exception as e:
        print(f"⚠️ No existing master.csv for {section_name} (creating new). Reason: {e}")
        df_combined = df_new

    out_buf = StringIO()
    df_combined.to_csv(out_buf, index=False, header=False)
    master_blob.upload_from_string(out_buf.getvalue(), content_type='text/csv')
    print(f"✅ Updated master.csv for {section_name}")
...
def process_pdf_with_pdfplumber(pdf_path, id_sec_a):
    ...
    append_to_gcs_master(df_a, "section_a")
    append_to_gcs_master(df_b1, "section_b1")
    append_to_gcs_master(df_b2, "section_b2")
    append_to_gcs_master(df_c, "section_c")
    append_to_gcs_master(df_d, "section_d")
    append_to_gcs_master(df_e1, "section_e1")
    append_to_gcs_master(df_e2, "section_e2")
